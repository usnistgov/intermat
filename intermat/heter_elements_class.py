...
def get_interface(
    ...
    tmp_abc = het['interface'].lattice.abc
    a = tmp_abc[0]
    b = tmp_abc[1]
    #print('het1\n')
    #print(het['interface'])
    tmp = subs_surf
    coords = subs_surf.cart_coords
    # print('disp',disp)
    # print('coords1\n')
    # print(coords)
    coords[:, 0] += disp[0]*a
...
class InterfaceCombi(object):
    def __init__(
        ...
        self.film_jids = film_jids
        self.subs_jids = subs_jids
    ...
    def generate(self):
        count = 0
        cwd = str(os.getcwd())
        for ii in range(len(subs_jids)):
            for jj in range(len(film_jids)):
                i = jids[ii]
                j = jids[jj]
                if count < 20000 and i != j and ii > jj:
                    ...
                    for seperation in seperations:
                        for film_index in film_indices:
                            for subs_index in subs_indices:
                                for thickness in thicknesses:
                                    for dis in xy:
                                        dis_tmp = dis
                                        thickness = round(
                                            thickness, rount_digit
                                        )
                                        seperation = round(
                                            seperation, rount_digit
                                        )
                                        dis_tmp[0] = round(
                                            dis_tmp[0], rount_digit
                                        )
                                        dis_tmp[1] = round(
                                            dis_tmp[1], rount_digit
                                        )
                                        print(i, j)
                                        ...
                                        if ats.num_atoms < 500:
                                            ...
                                            v = VaspJob(
                                                ...
                                                kpoints=kp,
                                                copy_files=[
                                                    "/users/knc6/bin/vdw_kernel.bindat"
                                                ],
                                                jobname=name,

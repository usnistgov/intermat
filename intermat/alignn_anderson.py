...
device='cpu'
dft_3d = data('dft_3d')
def get_model(model_path="/mnt/c/Users/knc6/OneDrive - NIST/KamalLaptop/JARVIS-CHIPS/ALIGNN/Out_CBM_3D"):
...
def get_surface(jid='',index=[]):
    ...
    return surf
model_cbm=get_model()
model_vbm=get_model("/mnt/c/Users/knc6/OneDrive - NIST/KamalLaptop/JARVIS-CHIPS/ALIGNN/Out_VBM_3D")
model_evac=get_model("/mnt/c/Users/knc6/OneDrive - NIST/KamalLaptop/JARVIS-CHIPS/ALIGNN/Out_phi_opt2")
##########################
...
count=10 #10
from jarvis.db.jsonutils import loadjson
dat=loadjson('/mnt/c/Users/knc6/OneDrive - NIST/KamalLaptop/JARVIS-CHIPS/WF/bulk_phi2.json')
...
for ii  in dat:
  ...
  miller=ii['name'].split('miller_')[1].split('_thickness')[0].replace('_','')
  atoms=Atoms.from_dict(ii['atoms'])
  ano=jid
  
  if jid in include_jids and 'JVASP-8118_miller_0_0_1' not in ii['name'] and 'JVASP-91_miller_1_1_0' not in ii['name'] and 'JVASP-91_miller_1_1_1' not in ii['name'] and 'JVASP-1002_1_0_0' not in ii['name'] and 'x' not in ii['name'] and '1180' not in ii['name'] and 'Surface-JVASP-1195_miller_0_0_1_' not in ii['name'] and 'Surface-JVASP-1002_miller_1_0_0_thickness_1' not in ii['name']:
      gap,formula=get_gap(jid)
     #if ano==proto:
      label=jid.replace('JVASP',formula)+' ('+miller+')' #ii['name'].split('_thickness')[0].split('Surface-')[1].split('JVASP-')[1].replace('_miller','')
      labels.append(label)
      ...
      print(ii['name'],atoms.composition.reduced_formula,alignn_evac,x_vbm,vbm,cbm)
      #print (vbm,cbm,gap)
...
# print('MAE,MAD',mean_absolute_error(x,y), mean_absolute_error(x,[np.array(x).mean() for i in range(len(x))]))       
...
print('Acc,MAE,MAD',accuracy_score(type_dft, type_dl),mean_absolute_error(x,y), mean_absolute_error(x,[np.array(x).mean() for i in range(len(x))]))       

...
def add_atoms(
    ...
    strain_x = (
        top.lattice.abc[0] - bottom.lattice.abc[0]
    ) / bottom.lattice.abc[0]
    strain_y = (
        top.lattice.abc[1] - bottom.lattice.abc[1]
    ) / bottom.lattice.abc[1]
    if apply_strain:
        top = top.strain_atoms([strain_x, strain_y, 0])
...
class InterfaceCombi(object):
    ...
    def make_interface(
        ...
        # print('film_scell props',film_scell.props)
        # print('subs_scell props',subs_scell.props)
        interface = add_atoms(
            film_scell,
            subs_scell,
            shift_normal,
        ...
        combined = interface
        # .center(vacuum=vacuum).center_around_origin(
        #    [0, 0, 0.5]
        # )
        if self.rotate_xz:
